...
// Items Management with theme support and smart dropdown positioning

'use client';
import React, { useState, useEffect, useRef } from 'react';
import { Package, Plus, Edit2, Trash2, X, Search, Filter } from 'lucide-react';
...
const ItemsManagement = () => {
  ...
  const filteredItems = items.filter(item => {
    ...
    const priceMin = filters.priceRange.min ? parseFloat(filters.priceRange.min) : null;
    const priceMax = filters.priceRange.max ? parseFloat(filters.priceRange.max) : null;
    const matchesPrice = (priceMin === null || item.mrp >= priceMin) &&
                        (priceMax === null || item.mrp <= priceMax);
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-600 text-sm mt-0.5">Manage your inventory items</p>
              </div>
            </div>
          </div>
          <Card>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900">Items Management</h1>
                <p className="text-slate-600 text-sm mt-0.5">Manage your inventory items and pricing</p>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  setSortBy(sort as 'name' | 'price' | 'gst');
              ...
              >
                ...
                <option value="gst-desc">GST (High to Low)</option>
                <option value="gst-asc">GST (Low to High)</option>
              </Select>
            </div>
          </div>
        </Card>
          ...
          {loading ? (
            ...
            <div className="p-12">
              <EmptyState
                ...
                description={searchTerm || hasActiveFilters ? "No items match your search or filters." : "Start by adding your first item."}
                action={
              ...
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-600 uppercase hidden md:table-cell">Category</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-600 uppercase hidden lg:table-cell">HSN</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-600 uppercase hidden sm:table-cell">GST</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-600 uppercase">MRP</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-600 uppercase hidden lg:table-cell">Retail</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-600 uppercase hidden lg:table-cell">Wholesale</th>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50">
                      <td className="px-4 py-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{item.unit?.name} ({item.unit?.abbreviation})</p>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-600 hidden md:table-cell">
                        {item.category?.name || 'N/A'}
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-600 hidden lg:table-cell">{item.hsn_code}</td>
                      <td className="px-4 py-3 hidden sm:table-cell">
                        <Badge variant="primary" size="sm">
                          {item.gst_rate}%
                        </Badge>
                      </td>
                      <td className="px-4 py-3 text-sm font-medium text-slate-900">₹{item.mrp}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 hidden lg:table-cell">₹{item.retail_price}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 hidden lg:table-cell">₹{item.wholesale_price}</td>
                        ...
                        <div className="relative" ref={el => { dropdownRefs.current[item.id] = el; }}>
                          ...
                          {openDropdown === item.id && (
                            <>
                              ...
                              />
                              <div 
                                className={`absolute right-0 w-48 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-20 ${
          ...
          <div className="min-h-screen w-full flex items-center justify-center py-8">
            <Card className="w-full max-w-2xl">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setShowFilterModal(false)} className="p-2 rounded-lg hover:bg-slate-100">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="space-y-6">
                ...
                {/* GST Rate Filter */}
                <div className="pt-6 border-t border-slate-200">
                  <h4 className="font-semibold text-slate-900 mb-3">GST Rate</h4>
                  <div className="flex flex-wrap gap-2">
                    {gstRateOptions.map(rate => (
                      <button
                        key={rate}
                        onClick={() => {
                          if (filters.gstRates.includes(rate)) {
                        ...
                        className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                          filters.gstRates.includes(rate)
                            ? `${theme.classes.bgPrimary} text-white`
                            : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                        }`}
                      >
          ...
          <div className="min-h-screen w-full flex items-center justify-center py-8">
            <Card className="w-full max-w-2xl">
              ...
              <div className="space-y-6">
                ...
                {/* Tax Info */}
                <div className="pt-6 border-t border-slate-200">
                  <h4 className="font-medium text-slate-900 mb-3">Tax Information</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <Input
                      label="HSN Code"
                ...
                {/* Stock */}
                <div className="pt-6 border-t border-slate-200">
                  <h4 className="font-medium text-slate-900 mb-3">Stock Settings</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <Input
          ...
          <div className="min-h-screen w-full flex items-center justify-center py-8">
            <Card className="w-full max-w-3xl">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-slate-900 mt-1">{viewingItem.name}</p>
                    </div>
                    <div>
                      <p className="text-slate-600">Category</p>
                      <p className="font-medium text-slate-900 mt-1">{viewingItem.category?.name || 'N/A'}</p>
                    </div>
                    <div>
                    ...
                    {viewingItem.description && (
                      <div className="col-span-2">
                        ...
                        <p className="font-medium text-slate-900 mt-1">{viewingItem.description}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* Tax Info */}
                <div className="pt-6 border-t border-slate-200">
                  <h4 className="font-semibold text-slate-900 mb-4">Tax Information</h4>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="text-slate-600">HSN Code</p>
                      <p className="font-medium text-slate-900 mt-1">{viewingItem.hsn_code || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-slate-600">GST Rate</p>
                      <p className="font-medium text-slate-900 mt-1">{viewingItem.gst_rate}%</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-slate-200">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <Card padding="sm" className="bg-slate-50">
                      <p className="text-xs text-slate-600 mb-1">MRP</p>
                      <p className="text-lg font-bold text-slate-900">₹{viewingItem.mrp}</p>
                    </Card>
                    <Card padding="sm" className="bg-slate-50">
                      <p className="text-xs text-slate-600 mb-1">Retail Price</p>
                      <p className="text-lg font-bold text-slate-900">₹{viewingItem.retail_price}</p>
                    </Card>
                    <Card padding="sm" className="bg-slate-50">
                      <p className="text-xs text-slate-600 mb-1">Wholesale Price</p>
                      <p className="text-lg font-bold text-slate-900">₹{viewingItem.wholesale_price}</p>
                    </Card>
                  </div>
                  <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="text-slate-600">Discount</p>
                      <p className="font-medium text-slate-900 mt-1">{viewingItem.discount_percent}%</p>
                    </div>
                    <div>
                      <p className="text-slate-600">Min Stock Level</p>
                      <p className="font-medium text-slate-900 mt-1">{viewingItem.min_stock_level}</p>
                    </div>
                  </div>
                </div>
                ...
                {/* Transaction History */}
                <div className="pt-6 border-t border-slate-200">
                  <h4 className="font-semibold text-slate-900 mb-4">Transaction History</h4>
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <h5 className="text-sm font-semibold text-slate-900">Purchases</h5>
                            ...
                            {itemTransactions.purchases.map((purchase, idx) => (
                              <div key={idx} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg text-sm">
                                ...
                                <div className="text-right">
                                  <p className="font-semibold text-slate-900">₹{purchase.total_price}</p>
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <h5 className="text-sm font-semibold text-slate-900">Sales</h5>
                          ...
                          <div className="space-y-2 max-h-64 overflow-y-auto">
                            {itemTransactions.sales.map((sale, idx) => (
                              <div key={idx} className="flex items-center justify-between p-3 bg-green-50 rounded-lg text-sm">
                                ...
                                <div className="text-right">
                                  <p className="font-semibold text-slate-900">₹{sale.total_price}</p>
                                </div>
                              </div>
                            ))}
                          </div>
